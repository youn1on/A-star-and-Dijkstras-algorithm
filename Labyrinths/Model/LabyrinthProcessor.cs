...
{
    public List<Vertice> GetVerticeList(int[][] labyrinth, int startPointX, int startPointY, int endPointX, int endPointY)
    {
    ...
    public static int[][] GetDistanceMatrix(List<Vertice> vertices)
    {
        ...
        for (int i = 0; i < vertices.Count; i++)
        {
            ...
            foreach (int adjacent in vertices[i].AdjacentVertices)
            {
                ...
                distanceMatrix[i][adjacent] = distanceMatrix[adjacent][i] = GetDistance(vertices[i], vertices[adjacent]);
            }
        }
